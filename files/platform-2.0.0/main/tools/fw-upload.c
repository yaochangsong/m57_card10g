...
int check(char *imagefile, int offset, int len, char *err_msg, char *mtd_name)
{
   ...
   // unsigned long checksum;
    ...
    int size_len = 0;
    if(!strcmp(mtd_name, "\"jffs2\"")){
        size_len = sizeof(jffs2_header_t);
        jffs2_header_t header;
        jffs2_header_t *hdr = &header;
        memcpy (hdr, ptr, sizeof(jffs2_header_t));
        if (ntohl(hdr->magic_bitmask) != JFFS2_BITMASK) {
            munmap(ptr, len);
            close(ifd);
            sprintf (err_msg, "Bad Magic Bitmask: \"%s\" is no valid image\n", imagefile);
            ret = 0;
            goto err;
        }
    } else if (!strcmp(mtd_name, "\"boot\"")){
        size_len = sizeof(boot_header_t);
        boot_header_t header;
        boot_header_t *hdr = &header;
        memcpy (hdr, ptr, sizeof(boot_header_t));
        for(int i = 0; i< (sizeof(hdr->arm_vector_table) / sizeof(hdr->arm_vector_table[0])); i++){
            if (ntohl(hdr->arm_vector_table[i]) != BOOT_ARM_VEC_TAB) {
                sprintf (err_msg, "Bad arm vector table: \"%s\" is no valid image\n", imagefile);
                ret = 0;
                goto err;
            }
        }
        if (ntohl(hdr->wdw) != WIDTH_DECT_WORD) {
                sprintf (err_msg, "Bad Width Detection Word: \"%s\" is no valid image\n", imagefile);
                ret = 0;
                goto err;
         }
         if (ntohl(hdr->header_sign) != HEADER_SIGN) {
                sprintf (err_msg, "Bad Header sign: \"%s\" is no valid image\n", imagefile);
                ret = 0;
                goto err;
         }
    }

    if ((unsigned)len < size_len) {
...
int main (int argc, char *argv[])
{
    ...
    D("_mtd_name=%s\n", _mtd_name);
#if 1
    if(!check(user_filename, 0, file_end - file_begin, err_msg, _mtd_name) ){
