...
static int client_srv_send_request(struct uh_client *cl)
{
    ...
    const char *url = cl->srv_request.url;
    char host[256];
    const char *data= cl->srv_request.data;
    int data_len = cl->srv_request.content_length;
    /* reset result code */
    cl->srv_request.result_code = -1;
    snprintf(host, sizeof(host) - 1, "%s:%d", cl->get_peer_addr(cl), cl->get_peer_port(cl));
    ...
    j += sprintf(http_headers+j," %s", url);
...
static int client_assemble_post(struct uh_client *cl, const char *url, const char *data)
{
    printf_debug("send post on list:%s, port=%d\n",  cl->get_peer_addr(cl), cl->get_peer_port(cl));
    cl->srv_request.method = UH_HTTP_METHOD_POST;
    cl->srv_request.url = url;
...
static int client_assemble_get(struct uh_client *cl, const char *url, const char *data)
{
    printf_debug("send post on list:%s, port=%d\n",  cl->get_peer_addr(cl), cl->get_peer_port(cl));
...
*/
static int client_srv_send(struct uh_client *client, const char *url, const char *data)
{
    if(data && strlen(data) > 0)
        client_assemble_post(client, url, data);
    else
        client_assemble_get(client, url, data);
...
void uh_client_srv_send(char *url, char *data, struct sockaddr_in *addr)
{
        ...
        list_for_each_entry_safe(cl_list, list_tmp, &srv->clients, list){
            if(addr != NULL){
                if(memcmp(&cl_list->peer_addr.sin_addr, &addr->sin_addr, sizeof(addr->sin_addr)) == 0){
                    client_srv_send(cl_list, url, data); 
                }
            } else {
                client_srv_send(cl_list, url, data);
            }
...
static void client_free(struct uh_client *cl)
{
    ...
    if (cl) {
        ...
        if(cl->fsctx && cl->fsctx->ops->fs_close){
            cl->fsctx->ops->fs_close(cl->fsctx);
            cl->fsctx->client = NULL;
        }
#endif
        free(cl);
        cl = NULL;
...
static void parse_var(struct uh_client *cl, char *query)
{
    ...
    while (query && *query) {
        ...
        if (*k && v){
            /* add url decode */
            if (uh_urldecode(val, sizeof(val) - 1, v, strlen(v)) < 0){
                return;
            }
            kvlist_set(kv, k, val);
        }
...
void uh_accept_client(struct uh_server *srv, bool ssl)
{
    ...
    cl->send_str2client = client_srv_send;
