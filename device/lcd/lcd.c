...
int8_t init_lcd(void)
{
    ...
    for(i = 0; i<MAX_RADIO_CHANNEL_NUM; i++){
        lcd_printf(EX_MID_FREQ_CMD, EX_MUTE_SW,   &poal_config->multi_freq_point_param[i].points[0].noise_en, NULL);
        lcd_printf(EX_MID_FREQ_CMD, EX_DEC_BW,   &poal_config->multi_freq_point_param[i].points[0].d_bandwith, NULL);
        lcd_printf(EX_MID_FREQ_CMD, EX_DEC_METHOD,   &poal_config->multi_freq_point_param[i].points[0].d_method, NULL);
        lcd_printf(EX_MID_FREQ_CMD, EX_RF_MODE_CODE,   &poal_config->rf_para[i].rf_mode_code, NULL);
        lcd_printf(EX_MID_FREQ_CMD, EX_RF_MID_FREQ,   &poal_config->rf_para[i].mid_freq, NULL);
        lcd_printf(EX_MID_FREQ_CMD, EX_RF_MGC_GAIN,   &poal_config->rf_para[i].mgc_gain_value, NULL);
    }
    return 0;
}
