...
wait:
   ...
   // sleep(1);
    ...
    for_each_set_bit(ch, get_ch_bitmap(CH_TYPE_FFT), MAX_RADIO_CHANNEL_NUM){
        if(ch >= MAX_RADIO_CHANNEL_NUM){
            printf_err("channel[%zd] is too big\n", ch);
        }
        printf_note("ch: %zd,Fixed mode FFT start\n", ch);
...
loop:   printf_note("######channel[%d] wait to deal work######\n", ch);
...
#if defined(CONFIG_BSP_WD_XCR) || defined(CONFIG_BSP_40G)
        if(poal_config->channel[ch].enable.map.enable == 0)
        ...
        printf_note("receive notify, [Channel:%d]%s Work: [%s], [%s], [%s]\n", 
                     ch,
                     poal_config->channel[ch].enable.map.enable == 0 ? "Stop" : "Start",
                     poal_config->channel[ch].enable.map.bit.fft == 0 ? "Psd Stop" : "Psd Start",
                     poal_config->channel[ch].enable.map.bit.audio == 0 ? "Audio Stop" : "Audio Start",
                     poal_config->channel[ch].enable.map.bit.iq == 0 ? "IQ Stop" : "IQ Start");
        ...
        for(;;){
            switch(poal_config->channel[ch].work_mode)
            {
                ...
                {   
                    ...
                    if(poal_config->channel[ch].enable.map.bit.fft || poal_config->channel[ch].enable.map.bit.analy){
                ...
                case OAL_MULTI_POINT_SCAN_MODE:
                {
                    ...
                    if(poal_config->channel[ch].enable.map.enable){
