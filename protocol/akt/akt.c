...
static int akt_execute_set_command(void)
{
    ...
    switch (header->code)
    {
        ...
        case DIRECTION_MULTI_FREQ_ZONE_CMD:
        {
            ...
            if(akt_convert_oal_config(pakt_config->enable.cid, DIRECTION_MULTI_FREQ_ZONE_CMD) == -1){
                err_code = RET_CODE_PARAMTER_NOT_SET;
                goto set_exit;
            }
        ...
        case MULTI_FREQ_DECODE_CMD:
        {
            ...
            executor_set_command(EX_MID_FREQ_CMD, EX_DEC_METHOD, NULL);
            config_save_batch(EX_WORK_MODE_CMD, EX_MULTI_POINT_SCAN_MODE, poal_config);
            break;
        }
        ...
        default:
          printf_err("not support class code[%d]\n",header->code);
          err_code = RET_CODE_PARAMTER_ERR;
          goto set_exit;
    }
set_exit:
