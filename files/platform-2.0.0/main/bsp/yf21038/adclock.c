...
void volume_set(intptr_t base,uint8_t dat, uint8_t ch)//dat 0~100
{
	uint8_t buf[2];
#if 0
	buf[0] = 0xe0;
...
void ad9467_int(volatile char * Spi_synth,uint8_t cs)
{
    spi_wrapper(Spi_synth,0x0,0x3c,cs);
    usleep(5000);
    spi_wrapper(Spi_synth,0x0,0x18,cs);
    usleep(5000);
    spi_wrapper(Spi_synth,0x8,0x00,cs);
    spi_wrapper(Spi_synth,0xd,0x00,cs);
    spi_wrapper(Spi_synth,0xf,0x00,cs);
    spi_wrapper(Spi_synth,0x10,0x00,cs);
    spi_wrapper(Spi_synth,0x14,0x09,cs);
    spi_wrapper(Spi_synth,0x15,0x00,cs);
    spi_wrapper(Spi_synth,0x16,0x00,cs);
    spi_wrapper(Spi_synth,0x18,0x00,cs);
    spi_wrapper(Spi_synth,0x2c,0x00,cs);
    spi_wrapper(Spi_synth,0x36,0x22,cs);
    spi_wrapper(Spi_synth,0xff,0x01,cs);
};
...
static int clock_adc_fpga_init(void)
{
    ...
    //apb_i2c_setclk(ADC_BASE,100000);
    //apb_spi_reset(ADC_BASE,1);
...
//clock_type: 0:内参考 1：外参考 2：外时钟
static int clock_adc_switch_ref(uint8_t clock_type)
{
    if (!ca_ctx.vir_addr) {
        return -1;
    }
    ...
    switch (clock_type) {
        case 0:
            printf_note(">>>>>>switch internal ref!\n");
            spi_wrapper(ca_ctx.vir_addr,0x001c,0x44,CLK_CS);
            spi_wrapper(ca_ctx.vir_addr,0x0232,0x01,CLK_CS);
            spi_wrapper(ca_ctx.vir_addr,0x0018,0x07,CLK_CS);
            spi_wrapper(ca_ctx.vir_addr,0x0232,0x01,CLK_CS);
            break;

       case 1:
            printf_note(">>>>>>switch external ref!\n");
            SET_ADC_CLOCK_REF(get_fpga_reg(), 0);  //0:外参考
            usleep(50);
            spi_wrapper(ca_ctx.vir_addr,0x001c,0x02,CLK_CS);
            spi_wrapper(ca_ctx.vir_addr,0x0232,0x01,CLK_CS);
            spi_wrapper(ca_ctx.vir_addr,0x0018,0x07,CLK_CS);
            spi_wrapper(ca_ctx.vir_addr,0x0232,0x01,CLK_CS);
            break;
       ...
       case 2:
            printf_note(">>>>>>switch external clock!\n");
            SET_ADC_CLOCK_REF(get_fpga_reg(), 1);//1：外时钟
            usleep(50);
            spi_wrapper(ca_ctx.vir_addr,0x0010,0x7d,CLK_CS);
            spi_wrapper(ca_ctx.vir_addr,0x0191,0x80,CLK_CS);
            spi_wrapper(ca_ctx.vir_addr,0x019c,0x30,CLK_CS);
            spi_wrapper(ca_ctx.vir_addr,0x01a1,0x30,CLK_CS);
            spi_wrapper(ca_ctx.vir_addr,0x01e1,0x01,CLK_CS);//ext vco
            spi_wrapper(ca_ctx.vir_addr,0x0232,0x01,CLK_CS);
            break;
       default:
            break;
