...
static int8_t _rs485_assemble_send_data(uint8_t **buffer, uint8_t cmd, void *data, size_t len)
{
    ...
    if(data && len > 0){
        if(len == sizeof(int32_t)){
            ...
            printf_err("NOT Support Data Type:[len=%ld]\n", len);
            return -1;
        }
        memcpy(fdata->data, (void *)&idata, len);
    }
    pend = fdata->data + len;
    *pend = FRAME_END;
    *buffer = (uint8_t *)fdata;
    return n;
}
...
int8_t rs485_com_set_v2(int32_t cmd, void *pdata)
{
    ...
    n = _rs485_assemble_send_data_v2((uint8_t **)&buffer, cmd, pdata);
    if(n > 0 && n < FRAME_MAX_LEN){
...
#ifdef SW_TO_UART2_WRITE
        SW_TO_UART2_WRITE();
#endif
        rs485_send_data_by_serial((uint8_t *)buffer, n);
    }else{
...
int elec_compass1_com_get(void *data, int time_sec_ms)
{
    ...
    do{
        ...
        uart_read_timeout(UART_COMPASS2_30_1350M_CODE, buffer, TIME_OUT_RESOLUTION);
        if (nbyte > 0)
        {
...
#ifdef SW_TO_UART0_WRITE
    SW_TO_UART0_WRITE();
#endif
    uart_send_data(UART_COMPASS2_30_1350M_CODE, (uint8_t *)fdata, 5);
    //uart_compass1_send_data((uint8_t *)fdata, 5);
    usleep(20000);  //等待发送完毕，在切换方向
#ifdef SW_TO_UART0_READ
    SW_TO_UART0_READ();
#endif
    free(fdata);
...
int elec_compass2_com_get(void *data, int time_sec_ms)
{
    ...
    do{
        nbyte = uart_read_timeout(UART_COMPASS_1_6G_CODE, buffer, TIME_OUT_RESOLUTION);
...
#ifdef SW_TO_UART1_WRITE
    SW_TO_UART1_WRITE();
#endif
    uart_send_data(UART_COMPASS_1_6G_CODE, (uint8_t *)HPR_CMD, strlen(HPR_CMD));
    //uart_compass2_send_data((uint8_t *)HPR_CMD, strlen(HPR_CMD));
    usleep(50000);  //等待发送完毕，在切换方向
#ifdef SW_TO_UART1_READ
    SW_TO_UART1_READ();
#endif
