...
#define  send_data_by_serial(buf, len)    uart_send_data(UART_LCD_CODE, buf, len)#else#define  send_data_by_serial(buf, len)#endif
#define SCREEN_BUTTON_PRESS_DOWN 1
#define  LOCAL_CONTROL_MODE()       (config_get_control_mode() == SCREEN_CTRL_LOCAL)#define  REMOTE_CONTROL_MODE()      (config_get_control_mode() == SCREEN_CTRL_REMOTE)#include "config.h"
...
typedef enum _SCREEN_CHANNEL_ADDR_TYPE {
    ...
    SCREEN_CHANNEL_FRQ2    = 0x05,
     SCREEN_CHANNEL_RF_GAIN = 0x06,     SCREEN_CHANNEL_GAIN_MODE = 0x07,     SCREEN_CHANNEL_NOISE_EN = 0x08,     SCREEN_CHANNEL_NOISE_VAL = 0x09,    SCREEN_CHANNEL_VOLUME  = 0x10,
    SCREEN_CHANNEL_MGC     = 0x11,
    SCREEN_CHANNEL_AU_CTRL = 0x12,
    SCREEN_CHANNEL_AU_BUTTON_CTRL  = 0x13,
     SCREEN_CHANNEL_MID_FREQ  = 0x15,}SCREEN_CHANNEL_ADDR_TYPE;
...
	SCREEN_LED_CLK_STATUS    = 0x0a,
	SCREEN_LED_AD_STATUS    = 0x09,
	//SCREEN_CPU_STATUS         = 0x0a,
	SCREEN_FPGA_STATUS        = 0x0b,
	SCREEN_CPU_DATA_STATUS    = 0x0c,
	SCREEN_FPGA_TEMP_STATUS   = 0x0d,  //采集卡温度
    SCREEN_RF0_LOCK_STATUS   = 0x10,  //通道1本振锁定    SCREEN_RF0_TEMP_STATUS   = 0x11,  //通道1射频温度    SCREEN_RF1_LOCK_STATUS   = 0x20,  //通道2本振锁定    SCREEN_RF1_TEMP_STATUS   = 0x21,  //通道2射频温度}SCREEN_COMMON_DATA2_SUB_ADDR_TYPE;

typedef enum _SCREEN_RF_FREQ_PARA {	
...
typedef enum _SCREEN_DECODE_PARA {
    ...
    SCREEN_DECODE_LSB   = 4,
    SCREEN_DECODE_USB   = 5,
    SCREEN_DECODE_ISB   = 6,    SCREEN_DECODE_PM    = 7,    SCREEN_DECODE_PULSE   = 8,    SCREEN_DECODE_IQ   = 9,}SCREEN_DECODE_PARA; 
typedef struct  _SCREEN_RF_FREQ_ST{
    uint8_t type;
    uint64_t idata;//float fdata;  //Hz}__attribute__ ((packed)) SCREEN_RF_FREQ_ST; typedef struct  _SCREEN_BW_PARA_ST{
    uint8_t type;
...
extern int8_t k600_scanf(uint8_t *pdata, int32_t total_len);
