...
static int json_write_file(char *filename, const cJSON *root)
{
    FILE *fp;
    char *printed_json = NULL;
    int do_format = 1;
...
static int json_write_config_param(cJSON* root, s_config *sconfig)
{
    ...
    for(int i = 0; i < cJSON_GetArraySize(if_parm); i++){
        ...
        json_write_array_int_param("spectrum_parm", "if_parm", i, "mute_thrh",config->channel[ch].multi_freq_point_param.points[0].noise_thrh);
    ...
    for(int i = 0; i < cJSON_GetArraySize(rf_parm); i++){
        ...
        json_write_array_double_param("spectrum_parm", "rf_parm", i, "board_mid_freq",config->channel[ch].rf_para.board_mid_freq);
    }
...
/* network */
        ...
        if(!strcmp("1.0", version->valuestring)){
            ...
            if(network != NULL){
                ...
                if(port!= NULL && cJSON_IsNumber(port)){
                    ...
                    config->network[index].ifname = CONFIG_NET_1G_IFNAME;
                }
            }
            ...
            if(network != NULL){
                ...
                if(port!= NULL && cJSON_IsNumber(port)){
                    ...
                    config->network[index].ifname = CONFIG_NET_10G_IFNAME;
                }
            }
            #endif
        }else{
	...
	value = cJSON_GetObjectItem(control_parm, "noise_level");
    if(value!= NULL && cJSON_IsNumber(value)){
        config->ctrl_para.noise_level = value->valueint;
        printf_debug("noise_level:%u, %u \n",value->valueint, config->ctrl_para.noise_level);
    }
...
/* calibration_parm */
   ...
   if(psd_freq!=NULL){
    ...
    value = cJSON_GetObjectItem(calibration, "lo_leakage_copy_points");
    if(value!= NULL && cJSON_IsNumber(value)){
        config->cal_level.lo_leakage.global_copy_points = value->valueint;
        printf_debug("lo_leakage_copy_points=>value is:%d\n",config->cal_level.lo_leakage.global_copy_points);
    }     
    /* lo_leakage */
...
/* spectrum_parm */
    ...
    /* bband_bw_factor */
    cJSON *bband_bw_factor = NULL;
    bband_bw_factor = cJSON_GetObjectItem(spectrum_parm, "bband_bw_factor");
    if(bband_bw_factor!=NULL){
        printf_debug("bband_bw_factor:\n");
        for(int i = 0; i < cJSON_GetArraySize(bband_bw_factor); i++){
            printfd("index:%d ", i);
            if(i >= ARRAY_SIZE(config->ctrl_para.bband_bw_factor)){
              break;
            }
            node = cJSON_GetArrayItem(bband_bw_factor, i);            
            value = cJSON_GetObjectItem(node, "bandwidth");
            if(cJSON_IsNumber(value)){
                config->ctrl_para.bband_bw_factor[i].bw_hz=value->valueint;
                printfd("[%d]bandwidth:%d, ", i, config->ctrl_para.bband_bw_factor[i].bw_hz);
            }
            value = cJSON_GetObjectItem(node, "extract");
            if(cJSON_IsNumber(value)){
               // printfd("rate:%d, ", value->valueint);
                config->ctrl_para.bband_bw_factor[i].extract=value->valueint;
                printfd("[%d]extract:%d, ", i, config->ctrl_para.bband_bw_factor[i].extract);
            }
            value = cJSON_GetObjectItem(node, "filter");
            if(cJSON_IsNumber(value)){
               // printfd("bandwidth:%d, ", config->ctrl_para.scan_bw.bindwidth_hz[i]);
                config->ctrl_para.bband_bw_factor[i].filter=value->valueint;
                printfd("[%d]filter:%d, ", i, config->ctrl_para.bband_bw_factor[i].filter);
            } 
			value = cJSON_GetObjectItem(node, "filter2");
            if(cJSON_IsNumber(value)){
               // printfd("bandwidth:%d, ", config->ctrl_para.scan_bw.bindwidth_hz[i]);
                config->ctrl_para.bband_bw_factor[i].filter2=value->valueint;
                printfd("[%d]filter2:%d, ", i, config->ctrl_para.bband_bw_factor[i].filter2);
            } 
             printfd("\n");
        }
    }
    /* niq_bw_factor */
    cJSON *niq_bw_factor = NULL;
    niq_bw_factor = cJSON_GetObjectItem(spectrum_parm, "niq_bw_factor");
    if(niq_bw_factor!= NULL){
        printf_debug("niq_bw_factor:\n");
        for(int i = 0; i < cJSON_GetArraySize(niq_bw_factor); i++){
            printfd("index:%d ", i);
            if(i >= ARRAY_SIZE(config->ctrl_para.niq_bw_factor)){
              break;
            }
            node = cJSON_GetArrayItem(niq_bw_factor, i);            
            value = cJSON_GetObjectItem(node, "bandwidth");
            if(cJSON_IsNumber(value)){
                config->ctrl_para.niq_bw_factor[i].bw_hz=value->valueint;
                printfd("[%d]bandwidth:%d, ", i, config->ctrl_para.niq_bw_factor[i].bw_hz);
            }
            value = cJSON_GetObjectItem(node, "extract");
            if(cJSON_IsNumber(value)){
               // printfd("rate:%d, ", value->valueint);
                config->ctrl_para.niq_bw_factor[i].extract=value->valueint;
                printfd("[%d]extract:%d, ", i, config->ctrl_para.niq_bw_factor[i].extract);
            }
            value = cJSON_GetObjectItem(node, "filter");
            if(cJSON_IsNumber(value)){
               // printfd("bandwidth:%d, ", config->ctrl_para.scan_bw.bindwidth_hz[i]);
                config->ctrl_para.niq_bw_factor[i].filter=value->valueint;
                printfd("[%d]filter:%d, ", i, config->ctrl_para.niq_bw_factor[i].filter);
            } 
			value = cJSON_GetObjectItem(node, "filter2");
            if(cJSON_IsNumber(value)){
               // printfd("bandwidth:%d, ", config->ctrl_para.scan_bw.bindwidth_hz[i]);
                config->ctrl_para.niq_bw_factor[i].filter2=value->valueint;
                printfd("[%d]filter2:%d, ", i, config->ctrl_para.niq_bw_factor[i].filter2);
            } 
             printfd("\n");
        }
    }
    /* dem_bw_factor */
    cJSON *dem_bw_factor = NULL;
    dem_bw_factor = cJSON_GetObjectItem(spectrum_parm, "dem_bw_factor");
    if(dem_bw_factor!=NULL){
        printf_debug("dem_bw_factor:\n");
        for(int i = 0; i < cJSON_GetArraySize(dem_bw_factor); i++){
            printfd("index:%d ", i);
            if(i >= ARRAY_SIZE(config->ctrl_para.dem_bw_factor)){
              break;
            }
            node = cJSON_GetArrayItem(dem_bw_factor, i);            
            value = cJSON_GetObjectItem(node, "bandwidth");
            if(cJSON_IsNumber(value)){
                config->ctrl_para.dem_bw_factor[i].bw_hz=value->valueint;
                printfd("[%d]bandwidth:%d, ", i, config->ctrl_para.dem_bw_factor[i].bw_hz);
            }
            value = cJSON_GetObjectItem(node, "extract");
            if(cJSON_IsNumber(value)){
               // printfd("rate:%d, ", value->valueint);
                config->ctrl_para.dem_bw_factor[i].extract=value->valueint;
                printfd("[%d]extract:%d, ", i, config->ctrl_para.dem_bw_factor[i].extract);
            }
            value = cJSON_GetObjectItem(node, "filter");
            if(cJSON_IsNumber(value)){
               // printfd("bandwidth:%d, ", config->ctrl_para.scan_bw.bindwidth_hz[i]);
                config->ctrl_para.dem_bw_factor[i].filter=value->valueint;
                printfd("[%d]filter:%d, ", i, config->ctrl_para.dem_bw_factor[i].filter);
            } 
			 value = cJSON_GetObjectItem(node, "filter2");
            if(cJSON_IsNumber(value)){
               // printfd("bandwidth:%d, ", config->ctrl_para.scan_bw.bindwidth_hz[i]);
                config->ctrl_para.dem_bw_factor[i].filter2=value->valueint;
                printfd("[%d]filter2:%d, ", i, config->ctrl_para.dem_bw_factor[i].filter2);
            } 
             printfd("\n");
        }
    }
    ...
    if(if_parm!=NULL){
        ...
        for(int i = 0; i < cJSON_GetArraySize(if_parm); i++){
            ...
            value = cJSON_GetObjectItem(node, "mute_thrh");
            if(cJSON_IsNumber(value)){
                config->channel[i].multi_freq_point_param.points[0].noise_thrh=value->valueint;
                printfd("noise_en[mute_thrh]:%d, ", config->channel[i].multi_freq_point_param.points[0].noise_thrh);
            } 
    ...
    if(rf_parm != NULL){
        ...
        for(int i = 0; i < cJSON_GetArraySize(rf_parm); i++){
            ...
            value = cJSON_GetObjectItem(node, "board_mid_freq");
            if(cJSON_IsNumber(value)){
                config->channel[ch].rf_para.board_mid_freq=value->valuedouble;
                printfd(" board_mid_freq:%"PRIu64"\n", config->channel[ch].rf_para.board_mid_freq);
            }
