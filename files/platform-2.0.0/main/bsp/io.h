...
struct  band_table_t{
    ...
    uint32_t filter_factor2;
    uint32_t band;
}; 


#ifdef CONFIG_DEVICE_HUMIDITY
    #ifdef CONFIG_DEVICE_HUMIDITY_SI7021
...
extern int32_t io_set_subch_dec_middle_freq(uint32_t ch, uint32_t subch, uint64_t dec_middle_freq, uint64_t middle_freq);
...
extern uint64_t io_get_raw_sample_rate(uint32_t ch, uint64_t middle_freq, uint32_t bw);
