...
int executor_net_disconnect_notify(struct sockaddr_in *addr)
{
    ...
    if(clinets == 0){
        ...
        for(ch = 0; ch< MAX_RADIO_CHANNEL_NUM; ch++){
            ...
            fs_ctx = get_fs_ctx();
            if(fs_ctx != NULL){
                fs_ctx->ops->fs_stop_save_file(ch, NULL, client->fsctx);
            }
#endif
#endif
        }
...
int8_t  executor_points_scan(uint8_t ch, work_mode_type mode, void *args)
{
    ...
    printf_note("residence_time=%d, points_count=%d, scan_bw=%u\n", point->residence_time, points_count, scan_bw);
        ...
        do{
            ...
            if((poal_config->channel[ch].enable.map.enable == 0) || 
               poal_config->channel[ch].enable.bit_reset == true){
                    printf_note("bit_reset...[%d, %d]\n", poal_config->channel[ch].enable.map.enable, poal_config->channel[ch].enable.bit_reset);
                    poal_config->channel[ch].enable.bit_reset = false;
                    return -1;
            }
#if defined (CONFIG_RESIDENCY_STRATEGY) 
